...
export default function ContactPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    border: "1.5px solid rgba(28,46,36,0.12)",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div className="glass-card" style={{ padding: "1.75rem", background: "var(--primary)" }}>
              ...
              <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.9rem", lineHeight: 1.7 }}>
                If you've made an EFT payment and your Honour Roll status hasn't updated after 2 business days, please email us with your payment reference number.
              </p>
            </div>
          </div>
          ...
          <div className="glass-card" style={{ padding: "2.25rem" }}>
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <h2 style={{ fontSize: "1.2rem", marginBottom: "0.25rem" }}>Send a Message</h2>
                <input type="text" required placeholder="Your Name *" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} style={inputStyle} />
                <input type="email" required placeholder="Email Address *" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} style={inputStyle} />
                <input type="text" placeholder="Subject" value={formData.subject} onChange={(e) => setFormData({ ...formData, subject: e.target.value })} style={inputStyle} />
                <textarea required rows={5} placeholder="Your message *" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} style={{ ...inputStyle, minHeight: "unset", resize: "vertical" }} />
